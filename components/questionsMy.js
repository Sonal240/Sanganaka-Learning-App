...
export default function AllQuestions(props) {
    ...
    const fetchquestions = () => {
        var art=[];
        db.collection('questions').where('mobile', '==', props.route.params.phno).orderBy('date', 'desc').limit(20).get().then(async (snapshot)=> {
            if(snapshot.docs.length) {
                art = await Promise.all(snapshot.docs.map(async (doc) => {
                    await db.collection('users').where('phno', '==', props.route.params.phno).get().then((snapshot)=> {
                        art={
        ...
        <>
            ...
            >
                <Card containerStyle={{padding: 0}} >
                    {
                        questions?questions.length?questions.map((item) => <DisplayList question={item} propsc={props} />):<Text>No Questions Found</Text>:<Loading />
                    }
                </Card>
                <View
                    style={{
