...
export default function AllQuestions(props) {
    ...
    const fetchquestions = () => {
        var art=[];
        db.collection('questions').where('mobile', '==', props.route.params.phno).orderBy('date', 'desc').limit(20).get().then((snapshot)=> {
    ...
    const fetchMorequestions = () => {
        if(!reachedEnd && !loading) {
            ...
            console.log(items);
            var art=[];
            db.collection('questions').where('mobile', '==', props.route.params.phno).orderBy('timestamp', 'desc').limit(items).get().then((snapshot)=> {
                snapshot.docs.map((doc) => {
